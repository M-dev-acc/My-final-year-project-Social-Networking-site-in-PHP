...
if (isset($_POST['verifyemail'])) {
	if (Database::runQuery("SELECT user_email FROM tbl_users WHERE user_email=:useremail", array(':useremail' => $_POST['user_email']))) {
        ...
        Mail::sendMail($_POST['user_email'], "Change your password.", "Click to reset you password <a href='http://localhost:7723/SocialPortal/settings.php?password_token=${token}'>http://localhost:7723/SocialPortal/settings.php?password_token=${token}</a>", );
	}else{
