...
if (isset($_GET['password_token'])) {
    ...
    if (Database::runQuery("SELECT password_token FROM tbl_password_tokens WHERE password_token=:token", array(':token' => md5($_GET['password_token'])))) {
        $isTokenValid = True;

        if (isset($_POST['changepassword'])) {
         if (strlen($_POST['new_password']) >= 6 && strlen($_POST['new_password']) <= 60) {
           if ($_POST['new_password'] == $_POST['repeat_password']) {
            $userid = Database::runQuery("SELECT user_id FROM tbl_password_tokens WHERE password_token=:token", array(':token' => md5($_GET['password_token'])))[0]['user_id'];
            ...
            Database::runQuery("DELETE FROM tbl_password_tokens WHERE password_token=:token", array(':token' => md5($_GET['password_token'])));
...
<form method="post" action="<?=($isTokenValid) ? 'settings.php?password_token='.$_GET['password_token'] : 'settings.php'; ?>" id="login_form" class="form push-down">
        <fieldset class="form__container">
            ...
            <div class="form-group-element">
                ...
                name="old_password"
                id="password-field"
                class="form__text-field password-field"
            ...
            <div class="form-group-element">
                ...
                maxlength="60"
                data-message_field="#user_password-message_field">
                <button class="form-group-element__button show_password_btn" id="show-hide-button" data-for="#user_password">&#128065;</button>
            </div>
            <div class="form-group-element">
                <input type="password"
...
Array.from(document.querySelectorAll('.show_password_btn')).forEach(showPasswordBtn => {
